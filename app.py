...
def render_dashboard():
    ...
    if 'last_run_results' in st.session_state:
        ...
        if not alerts:
            st.warning("No new alerts generated (suppressed by deduplication or no threats found)")
        else:
    ...
    if not df_all.empty:
        ...
        st.info("No data available. Run the detection engine to populate.")
...
def render_threat_hunter():
    ...
    with col1:
        nl_query = st.text_input("Enter threat hunt in natural language:", placeholder="e.g., 'Find PowerShell executions with network connections'")
    with col2:
        query_lang = st.selectbox("Query Language", ["KQL", "EQL", "SQL"])
        ...
        if nl_query:
            with st.spinner("Executing hunt..."):
                try:
                    ...
                    st.error(f"Threat Hunter Error: {str(e)}")
        else:
...
def render_dedup_stats():
    ...
    st.write("Our RAG-based engine identifies duplicate alerts to reduce fatigue.")
    
    # Placeholder for real stats if available
    st.metric("Known Patterns", "128")
    st.metric("Suppression Rate", "64.2%")
    
    st.info("The deduplicator learns from every alert processed, building a knowledge base of 'Known Good' and 'Known Bad' patterns.")

def render_threat_intel():
    st.title("🌐 Threat Intelligence Feed")
    if st.button("🔄 Refresh Feed"):
        with st.spinner("Fetching latest IOCs..."):
            threat_feed_service.update_feeds()
    Professional product UI upgrade, debugged Telegram, fixed Threat Hunter session state
...
def render_settings():
    ...
    st.info(f"Current Bot Token: `{bot_token[:5]}***{bot_token[-5:] if len(bot_token)>10 else ''}`")
    st.info(f"Current Chat ID: `{chat_id}`")
    ...
    if st.button("Send Test Notification"):
        if bot_token and chat_id:
            try:
                ...
                payload = {"chat_id": chat_id, "text": f"📣 SOC ALERT TEST
{test_msg}"}
